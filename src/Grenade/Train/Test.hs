...
accuracy net dataSet =
    let (inpt, labelShapes) = munzip dataSet
        outptShapes = runNet net <$> inpt
        nOfCorrectPredictions =
            length . NEL.filter (uncurry samePrediction) $
            mzip labelShapes outptShapes
     in case accuracyM $
...
samePrediction _ _ =
    error
        "NeuralNet.Test.samePrediction: The output and label are not 1D vectors"
