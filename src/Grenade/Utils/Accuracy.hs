...
data AccuracyOutOfBounds = AccuracyOutOfBounds deriving (Show, Eq, Generic)
...
accuracyM x = case prettyValidation $ Accuracy x of
    Right a -> pure a
...
showAccuracy name (Accuracy x) = "The " ++ name ++ " accuracy is "++  show (nOfPrecent * x) ++ "%."
