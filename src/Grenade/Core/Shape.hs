...
data Shape
  ...
  -- ^ Two dimensional matrix. Row, Column.
    | D3 Nat
         Nat
         Nat -- ^ Three dimensional matrix. Row, Column, Channels.
...
randomOfShape ::
       forall x m. (MonadRandom m, SingI x)
    => m (S x)
randomOfShape = do
    seed :: Int <- getRandom
    return $
        case (sing :: Sing x) of
            D1Sing SNat -> S1D (randomVector seed Uniform * 2 - 1)
            D2Sing SNat SNat -> S2D (uniformSample seed (-1) 1)
            D3Sing SNat SNat SNat -> S3D (uniformSample seed (-1) 1)
...
nk x =
    case (sing :: Sing x) of
        D1Sing SNat -> S1D (konst x)
        D2Sing SNat SNat -> S2D (konst x)
        D3Sing SNat SNat SNat -> S3D (konst x)

instance KnownNat n => Validity (R n) where
    validate v = delve "R-based vector" $ V.toList (extract v)

instance (KnownNat i, KnownNat j) => Validity (L i j) where
    validate m = delve "L-based matrix" . V.toList . NLA.flatten $ extract m
