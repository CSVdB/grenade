...
instance (KnownNat a, KnownNat x, KnownNat y, a ~ (x * y)) =>
         Layer Reshape ('D2 x y) ('D1 a) where
    type Tape Reshape ('D2 x y) ('D1 a) = ()
    runForwards _ (S2D y) =
        ((), fromJust' . fromStorable . flatten . extract $ y)
...
instance ( KnownNat a
         ...
         Layer Reshape ('D1 a) ('D3 x y z) where
    type Tape Reshape ('D1 a) ('D3 x y z) = ()
    runForwards _ (S1D y) = ((), fromJust' . fromStorable . extract $ y)
    runBackwards _ _ (S3D y) =
        ((), fromJust' . fromStorable . flatten . extract $ y)
